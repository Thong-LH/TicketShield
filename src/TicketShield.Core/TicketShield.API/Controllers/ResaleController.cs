...
public sealed class ResaleController(IServiceProvider services) : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> RequestOtp([FromBody] RequestOtpBody body, [FromHeader(Name = "Idempotency-Key")] string key, CancellationToken ct) => WorkflowResponse(await Workflow.Request(Seller, key, body.TicketCode, ct));
    [HttpPost("{id}/resend")]
    public async Task<IActionResult> Resend(string id, [FromHeader(Name = "Idempotency-Key")] string key, CancellationToken ct) => WorkflowResponse(await Workflow.Resend(Seller, id, key, ct));
    [HttpPost("{id}/confirm")]
    public async Task<IActionResult> Confirm(string id, [FromBody] ConfirmOtpBody body, [FromHeader(Name = "Idempotency-Key")] string key, CancellationToken ct) => WorkflowResponse(await Workflow.Confirm(Seller, id, key, body.Otp, ct));
    [HttpGet("{id}")]
    public async Task<IActionResult> Get(string id, CancellationToken ct) => WorkflowResponse(await Workflow.Get(Seller, id, ct));
    [HttpPost("{id}/close")]
    public async Task<IActionResult> Close(string id, [FromHeader(Name = "Idempotency-Key")] string key, CancellationToken ct) => WorkflowResponse(await Workflow.Close(Seller, id, key, ct));
    [HttpPost("/api/resale-listings")]
    public async Task<IActionResult> Publish([FromBody] PublishBody body, [FromHeader(Name = "Idempotency-Key")] string key, CancellationToken ct) => WorkflowResponse(await Workflow.Publish(Seller, key, body, ct));
...
public sealed class ResaleErrorsAttribute : ExceptionFilterAttribute
{
    public override void OnException(ExceptionContext context)
    {
        ...
        context.Result = new ObjectResult(ApiResponse<object>.FailureResponse(error.Code, [error.Code])) { StatusCode = error.HttpStatus };
